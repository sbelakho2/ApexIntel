...
const external = [];
const APP = 'https://app.apexmail.ee';
p.on('request', r => { const u = new URL(r.url()); if (u.origin !== APP && !u.href.includes('apexmail.ee')) external.push(r.url()); });
const cspViolations = [];
p.on('console', m => { if (m.type() === 'error' && m.text().includes('Content Security Policy')) cspViolations.push(m.text().slice(0, 80)); });
await p.goto(APP + '/login/', { waitUntil: 'load' });
const r = await p.evaluate(async ({ driver, wasm, css }) => {
  ...
  const s1 = document.createElement('script'); s1.textContent = wasm; document.body.appendChild(s1);
  const s2 = document.createElement('script'); s2.textContent = driver; document.body.appendChild(s2);
  const deadline = Date.now() + 30000;
  while (Date.now() < deadline) {
    const t = wrap.querySelector('[data-kiwi-token]');
    const w = document.querySelector('[data-kiwi-widget]');
    if (t && t.value) {
      const plain = atob(t.value);
      const parts = plain.split('.');
      let telemetry = {};
      try { telemetry = JSON.parse(parts[3] || '{}'); } catch { telemetry = { parse: 'fail' }; }
      const banned = ['hc', 'dm', 'sw', 'sh', 'et', 'wd'].filter(k => k in telemetry);
      return { state: w?.getAttribute('data-state'), counter: parts[1], telemetryKeys: Object.keys(telemetry), telemetry, bannedPresent: banned };
    }
    await new Promise(r2 => setTimeout(r2, 200));
  }
  return { state: document.querySelector('[data-kiwi-widget]')?.getAttribute('data-state'), timeout: true };
}, { driver, wasm, css });
console.log(JSON.stringify({ result: r, externalRequests: external, cspViolations: cspViolations.length }, null, 1));
await browser.close();
