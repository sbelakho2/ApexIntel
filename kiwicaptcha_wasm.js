 ...
 * @returns {number}
 */
export function solve_argon2_chunk(prefix, salt, target_bits, m_kib, t, p, start_counter, chunk_size) {
    ...
    _assertNum(t);
    _assertNum(p);
    _assertNum(start_counter);
    _assertNum(chunk_size);
    const ret = wasm.solve_argon2_chunk(ptr0, len0, ptr1, len1, target_bits, m_kib, t, p, start_counter, chunk_size);
    return ret;
}
 ...
 * @returns {number}
 */
export function solve_sha256_chunk(prefix, salt, target_bits, start_counter, chunk_size) {
    const ptr0 = passArray8ToWasm0(prefix, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArray8ToWasm0(salt, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    ...
    _assertNum(start_counter);
    _assertNum(chunk_size);
    const ret = wasm.solve_sha256_chunk(ptr0, len0, ptr1, len1, target_bits, start_counter, chunk_size);
    return ret;
}
