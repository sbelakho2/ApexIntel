...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'insights' AND column_name = 'title_hash') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_insights_title_hash ON insights(title_hash) WHERE title_hash IS NOT NULL';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'insights' AND column_name = 'entity_ids') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_insights_dedup_window ON insights(insight_type, created_at DESC) WHERE entity_ids IS NOT NULL';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'insights' AND column_name = 'entity_ids') THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_insights_entity_ids_gin ON insights USING gin(entity_ids) WHERE entity_ids IS NOT NULL';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
        ...
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_warnings_entity_ids_gin ON warnings USING gin(entity_ids) WHERE entity_ids IS NOT NULL';
    END IF;
END $$;
