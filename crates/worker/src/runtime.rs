...
#[tracing::instrument(skip(scheduler, store, ingress))]
    ...
    for kind in due {
        ...
        handles.push((
            ...
            tokio::spawn(async move {
                ...
                let mut job_handle =
                    tokio::spawn(async move { execute_job(&run_kind, &store, &ingress).await });
    ...
    loop {
        ...
        match store.pop_job_trigger().await {
            Ok(Some((trigger_id, job_kind_str))) => {
                ...
                tokio::spawn(async move {
                    let _permit = permit;
                    let run = execute_job(&kind, &store, &ingress).await;
                    persist_run_history(&store, &run).await;
