...
/// Maximum number of companies to process per invocation.
const MAX_COMPANIES_PER_RUN: usize = 50;
...
async fn load_recent_observations_by_company(
    ...
    let rows = sqlx::query_as::<_, ObsCompanyRow>(
        r#"SELECT DISTINCT ON (o.id)
                   ...
                   o.content
            FROM observations o
            JOIN observation_entity_graph eig ON o.id = eig.observation_id
            WHERE o.created_at >= $1
              AND COALESCE(o.content, '') != ''
              AND LENGTH(COALESCE(o.content, '')) >= $2
...
async fn generate_insights_for_company(
    ...
    let mut evidence_signals: Vec<EvidenceSignal> = observations
        ...
        .enumerate()
        .map(|(i, text)| EvidenceSignal {
            title: format!("Observation {}: {}", i + 1, truncate_for_title(text, 120)),
            description: text.clone(),
            source_url: "observation".to_string(),
    ...
    let category = infer_best_category(observations);
    ...
    let grounding_ratio = validate_grounding(&narrative, &recommendation, &evidence_signals);
...
fn infer_best_category(observations: &[String]) -> String {
    ...
    scores.push((
        "supply_chain_risk",
        score(&["supply", "chain", "shortage", "lead time", "supplier", "sourcing", "procurement"]),
    ));
    scores.push((
        "demand_procurement",
        score(&["contract", "award", "rfp", "tender", "order", "capacity", "expansion"]),
    ));
    scores.push((
        "competitor_market",
        score(&["competitor", "market share", "launch", "product", "win", "loss"]),
    ));
    scores.push((
        "security_compliance",
        score(&["breach", "cyber", "vulnerab", "compliance", "audit", "certif", "iso"]),
    ));
    scores.push((
        "geopolitical_analysis",
        score(&["sanction", "tariff", "export", "geopolitical", "embargo", "trade war"]),
    ));
    scores.push((
        "regulatory_policy",
        score(&["regulation", "policy", "law", "directive", "mandate", "restriction"]),
    ));

    scores.sort_by(|a, b| b.1.cmp(&a.1));
    if scores.first().map(|(_, s)| *s).unwrap_or(0) == 0 {
        "demand_procurement".to_string() // sensible default for B2B intel
    } else {
...
///
/// Uses the `SourceGroundingValidator` from `apex-llm` to check that key
/// entities (company name, POI names) mentioned in the output actually appear
/// in the source evidence — guarding against hallucinated names/facts.
#[cfg(feature = "llm")]
fn validate_grounding(
    ...
    grounded as f64 / sample_size as f64
}
