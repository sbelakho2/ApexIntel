...
-- The annotations table may already exist from a prior schema.
-- Only add parent_id column if missing, then create indexes safely.
...
-- If annotations already existed (e.g. from old schema), add parent_id column
ALTER TABLE annotations ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES annotations(id) ON DELETE CASCADE;

-- Conditionally create indexes only if the referenced columns exist
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='entity_type') THEN
        CREATE INDEX IF NOT EXISTS idx_annotations_entity ON annotations(entity_type, entity_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='parent_id') THEN
        CREATE INDEX IF NOT EXISTS idx_annotations_parent ON annotations(parent_id) WHERE parent_id IS NOT NULL;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='author_id') THEN
        CREATE INDEX IF NOT EXISTS idx_annotations_author ON annotations(author_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='is_resolved') THEN
        CREATE INDEX IF NOT EXISTS idx_annotations_resolved ON annotations(is_resolved) WHERE is_resolved = FALSE;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='created_at') THEN
...
CREATE INDEX IF NOT EXISTS idx_bookmarks_folder ON bookmarks(user_id, folder) WHERE folder IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_review_cycle_items_cycle ON review_cycle_items(cycle_id);
CREATE INDEX IF NOT EXISTS idx_review_cycle_items_assigned ON review_cycle_items(assigned_to) WHERE assigned_to IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_review_cycle_items_status ON review_cycle_items(status);
CREATE INDEX IF NOT EXISTS idx_review_cycle_items_due ON review_cycle_items(due_date) WHERE due_date IS NOT NULL;
...
-- Only create indexes if the columns exist (tags table may have different schema)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tags' AND column_name='name') THEN
        CREATE INDEX IF NOT EXISTS idx_tags_name ON tags(name);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tags' AND column_name='category') THEN
...
-- Only create indexes if the referenced columns exist (tag_assignments may have different schema)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tag_assignments' AND column_name='tag_id') THEN
        CREATE INDEX IF NOT EXISTS idx_tag_assignments_tag ON tag_assignments(tag_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tag_assignments' AND column_name='entity_type') THEN
...
DROP TRIGGER IF EXISTS update_notifications_prefs_updated_at ON notification_preferences;
...
DROP TRIGGER IF EXISTS update_review_cycle_items_updated_at ON review_cycle_items;
...
-- Composite index for common queries (only if columns exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='entity_type') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='annotations' AND column_name='author_id') THEN
        CREATE INDEX IF NOT EXISTS idx_annotations_entity_author ON annotations(entity_type, entity_id, author_id);
    END IF;
END $$;
