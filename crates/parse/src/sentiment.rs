...
fn get_positive_keywords(lang: &str) -> &'static [&'static str] {
    match lang {
        "en" => &[
            "growth", "expansion", "profit", "revenue", "innovation",
            "partnership", "award", "milestone", "record", "success",
            "investment", "acquisition", "upgrade", "breakthrough", "leading",
            "certification", "contract", "approved", "launch", "strategic",
        ],
        "fr" => &[
            "croissance", "expansion", "bénéfice", "chiffre d'affaires", "innovation",
            "partenariat", "récompense", "record", "succès", "investissement",
            "acquisition", "modernisation", "percée", "leader", "certification",
            "contrat", "approuvé", "lancement", "stratégique", "développement",
        ],
        "ar" => &[
            "نمو", "توسع", "ربح", "إيرادات", "ابتكار",
            "شراكة", "جائزة", "إنجاز", "نجاح", "استثمار",
            "استحواذ", "تطوير", "اختراق", "ريادة", "شهادة",
            "عقد", "موافقة", "إطلاق", "تعاون", "تقدم",
        ],
        "he" => &[
            "צמיחה", "הרחבה", "רווח", "הכנסות", "חדשנות",
            "שותפות", "פרס", "אבן דרך", "הצלחה", "השקעה",
            "רכישה", "שדרוג", "פריצת דרך", "מוביל", "הסמכה",
            "חוזה", "אישור", "השקה", "אסטרטגי", "פיתוח",
        ],
        "zh" => &[
            "增长", "扩张", "利润", "收入", "创新",
            "合作", "获奖", "里程碑", "成功", "投资",
        ...
        "de" => &[
            "Wachstum", "Expansion", "Gewinn", "Umsatz", "Innovation",
            "Partnerschaft", "Auszeichnung", "Meilenstein", "Erfolg", "Investition",
            "Übernahme", "Modernisierung", "Durchbruch", "führend", "Zertifizierung",
            "Auftrag", "Genehmigung", "Markteinführung", "strategisch", "Entwicklung",
        ],
        _ => &[],
    }
}
...
fn get_negative_keywords(lang: &str) -> &'static [&'static str] {
    match lang {
        "en" => &[
            "decline", "loss", "layoff", "closure", "bankruptcy",
            "recall", "lawsuit", "penalty", "violation", "downgrade",
            "delay", "shortage", "dispute", "sanction", "fraud",
            "default", "investigation", "restructuring", "warning", "risk",
        ],
        "fr" => &[
            "déclin", "perte", "licenciement", "fermeture", "faillite",
            "rappel", "procès", "amende", "violation", "dégradation",
            "retard", "pénurie", "litige", "sanction", "fraude",
            "défaut", "enquête", "restructuration", "avertissement", "risque",
        ],
        "ar" => &[
            "تراجع", "خسارة", "تسريح", "إغلاق", "إفلاس",
            "سحب", "دعوى", "غرامة", "انتهاك", "تخفيض",
            "تأخير", "نقص", "نزاع", "عقوبة", "احتيال",
            "تخلف", "تحقيق", "إعادة هيكلة", "تحذير", "مخاطر",
        ],
        "he" => &[
            "ירידה", "הפסד", "פיטורים", "סגירה", "פשיטת רגל",
            "ריקול", "תביעה", "קנס", "הפרה", "הורדת דירוג",
            "עיכוב", "מחסור", "סכסוך", "סנקציה", "הונאה",
            "חדלות פירעון", "חקירה", "ארגון מחדש", "אזהרה", "סיכון",
        ],
        "zh" => &[
            "下降", "亏损", "裁员", "关闭", "破产",
            "召回", "诉讼", "罚款", "违规", "降级",
        ...
        "de" => &[
            "Rückgang", "Verlust", "Entlassung", "Schließung", "Insolvenz",
            "Rückruf", "Klage", "Strafe", "Verstoß", "Herabstufung",
            "Verzögerung", "Engpass", "Streit", "Sanktion", "Betrug",
            "Ausfall", "Ermittlung", "Umstrukturierung", "Warnung", "Risiko",
        ],
        _ => &[],
    }
}
...
pub fn detect_language(text: &str) -> &'static str {
    ...
    for ch in text.chars() {
        let lang = match ch {
            ...
            '\u{00C0}'..='\u{00DE}' | '\u{00E0}'..='\u{00E3}'
            | '\u{00E5}'..='\u{00F5}' | '\u{00F7}'..='\u{00FB}'
...
mod tests {
    ...
    #[test]
    fn english_negative() {
        let result = analyze_sentiment("Company faces bankruptcy after massive layoff and fraud investigation", "en");
        assert!(result.score < 0.0);
    ...
    #[test]
    fn french_positive() {
        let result = analyze_sentiment("L'entreprise annonce une croissance record et un partenariat stratégique", "fr");
        assert!(result.score > 0.0);
    }
    ...
    #[test]
    fn german_keywords() {
        let result = analyze_sentiment("Das Unternehmen meldet Wachstum und eine neue Partnerschaft", "de");
        assert!(result.score > 0.0);
    }
}
