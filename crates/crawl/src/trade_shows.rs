...
fn event(
    ...
    location: &str,
    country: &str,
    sy: i32, sm: u32, sd: u32,
    ey: i32, em: u32, ed: u32,
    url: &str,
    tags: &[&str],
) -> TradeShowEvent {
    TradeShowEvent {
    ...
    for (event_name, exhibitors) in exhibitor_lists {
        ...
        for exhibitor in exhibitors {
            ...
            for poi in poi_names {
                if ex_lower.contains(&poi.to_lowercase())
                    || poi.to_lowercase().contains(&ex_lower)
                {
                    matches.push(ExhibitorMatch {
                        event_name: event_name.clone(),
                        event_dates: dates.clone(),
...
END:VCALENDAR";
        ...
        assert_eq!(events[0].start_date, NaiveDate::from_ymd_opt(2026, 3, 15).unwrap());
    }
