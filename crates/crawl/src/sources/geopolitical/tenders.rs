...
impl TenderSource {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SamGov => "sam_gov",
            Self::TedEu => "ted_eu",
            Self::FindATenderUk => "find_a_tender_uk",
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::SamGov => "US SAM.gov",
            Self::TedEu => "EU TED",
            Self::FindATenderUk => "UK Find a Tender",
...
impl TenderMonitor {
    ...
    fn parse_sam_gov_tender(&self, value: &serde_json::Value) -> Option<Tender> {
        let tender_id = value["noticeId"]
            .as_str()
            .or(value["uiId"].as_str())?
            .to_string();
        ...
        let keywords_matched: Vec<String> = self
            ...
            .iter()
            .filter(|kw| {
                title.to_lowercase().contains(&kw.to_lowercase())
