...
use crate::{api, components::cards::{PageHeader, StatCard, SurfaceCard}};
        ...
        <div class="page">
            ...
            <Suspense fallback=move || view! { <SurfaceCard title="Security" subtitle="Loading security summary."><p class="muted-copy">"Loading..."</p></SurfaceCard> }>
                {move || summary.get().map(|result| match result {
                    ...
                    Err(message) => view! { <SurfaceCard title="Security" subtitle="The API request failed."><p class="error-copy">{message}</p></SurfaceCard> }.into_view(),
                })}
            </Suspense>
        </div>
    }
