...
pub async fn list_competitors(
    ...
    let total = store.count_competitors().await.unwrap_or(competitor_rows.len() as i64);
    ...
    let mut competitors: Vec<CompetitorCard> = competitor_rows.iter().enumerate().map(|(i, c)| {
        let risk = c.risk_score.map(|s| (s * 100.0) as i64).unwrap_or(0).clamp(0, 100);
    ...
    let (change_rows, _total_changes) = store.get_all_competitor_changes_paged(1, 20).await.unwrap_or_else(|e| {
        tracing::error!("Failed to list competitor changes: {e}");
        (vec![], 0)
    });
    let recent_changes: Vec<CompetitorChange> = change_rows.iter().map(|ch| {
        CompetitorChange {
