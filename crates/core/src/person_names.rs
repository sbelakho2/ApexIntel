...
const FACILITY_SUFFIXES: &[&str] = &[
    "farm", "park", "plant", "station", "project", "phase", "unit", "field",
    "mine", "mill", "refinery", "terminal", "depot", "facility", "complex",
    "center", "centre", "tower", "dam", "reservoir", "pipeline", "grid",
    "line", "corridor", "zone", "district", "estate", "port", "airport",
    "harbor", "harbour", "warehouse", "factory", "works", "lab", "laboratory",
    "institute", "academy", "university", "school", "college", "hospital",
    "clinic", "mall", "plaza", "square", "boulevard", "avenue",
];
...
const PLACE_NAME_GAZETTEER: &[&str] = &[
    ...
    "ha nam", "ha tinh", "hai duong", "hai phong", "hau giang", "hanoi",
    ...
    // ── Morocco: regions ──
    "casablanca settat", "draa tafilalet", "fes meknes", "guelmim oud noun",
    "laayoune sakia el hamra", "marrakech safi", "oriental", "rabat sale kenitra",
    "souss massa", "tanger tetouan al hoceima", "beni mellal khenifra",
    "dakhla oued ed dahab", "casablanca", "rabat", "marrakech", "fez",
    "tangier", "agadir", "meknes", "oujda", "kenitra", "tetouan",
    "safi", "el jadida", "nador", "settat", "berkan", "khouribga",
    "mohammedia", "laayoune", "dakhla",
    ...
    "medenine", "tataouine", "gafsa", "tozeur", "kebili", "gabès",
    "médenine", "tatouine", "kairouan",
    // ── Czech Republic: regions ──
    "prague", "central bohemia", "south bohemia", "plzen", "karlovy vary",
    "usti nad labem", "liberec", "hradec kralove", "pardubice",
    "vysocina", "south moravia", "olomouc", "moravia silesia", "zin",
    "hradec kralove", "pardubice",
    // ── Major false-positive cities worldwide ──
    "sao paulo", "rio de janeiro", "buenos aires", "mexico city",
    "santiago", "bogota", "lima", "caracas", "havana",
    "istanbul", "ankara", "izmir", "bursa",
    "lagos", "nairobi", "addis ababa", "accra", "dakar", "abidjan",
    "riyadh", "dubai", "doha", "kuwait city", "muscat", "amman",
    ...
    "zurich", "geneva", "vienna", "prague", "warsaw",
...
pub fn is_facility_or_project_name(name: &str) -> bool {
    ...
    if words.iter().any(|w| energy_words.contains(&(*w))) {
        ...
        let generic_words = [
            "wind", "solar", "power", "energy", "gas", "thermal",
            "farm", "plant", "project", "group", "the", "and", "of",
        ];
        let has_proper_noun = words
            .iter()
            .any(|w| !generic_words.contains(&(*w)));
        if has_proper_noun {
            return true;
        }
    }
